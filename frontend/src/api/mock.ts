// Mock mode: serves the exact contract shapes from the shared seed fixture, and
// applies fix-queue actions to an in-memory copy so the demo behaves like a live
// system. client.ts switches here when VITE_API_URL is unset.
...
import { STAGES, TODAY, daysBetween, daysInStage, isLive, isOverdue } from '../lib/pipeline'
import { DEFAULT_SLA, SLA_DAYS } from '../lib/triage'

const TODAY_ISO = TODAY.toISOString().slice(0, 10)
let db: Invoice[] = JSON.parse(JSON.stringify(seedsRaw)) as Invoice[]
...
export function mockInvoices(params: {
  ...
  // Clones, not refs: the query cache must never hold objects our mutations
  // edit in place, or refetches look identical and the UI never updates.
  return delay(structuredClone(out))
}

export function mockInvoice(id: string): Promise<Invoice> {
  return delay(structuredClone(find(id)))
}

// ---- fix-queue actions (deterministic state transitions) ----

export function mockResubmit(id: string): Promise<Invoice> {
  const inv = find(id)
...
export function mockReply(id: string): Promise<Invoice> {
  const inv = find(id)
  inv.pipeline_status = 'with_insurer'
  inv.query_reason = null
  inv.timeline = [...inv.timeline, { stage: 'with_insurer', at: TODAY_ISO }]
  return delay({ ...inv })
}

export function mockSubmitDraft(id: string): Promise<Invoice> {
  const inv = find(id)
  inv.pipeline_status = 'at_medserv'
  inv.submitted_date = TODAY_ISO
  inv.timeline = [...inv.timeline, { stage: 'at_medserv', at: TODAY_ISO }]
...
export function mockChase(id: string): Promise<Invoice> {
  const inv = find(id)
  inv.last_chased_at = TODAY_ISO
  return delay({ ...inv })
}

export function mockResolve(id: string): Promise<Invoice> {
  const inv = find(id)
  inv.amount_due = 0
  return delay({ ...inv })
}
...
export function mockSummary(): Promise<Summary> {
  const owed = db.filter((i) => i.pipeline_status !== 'rejected' && i.amount_due > 0)
  const outstanding = owed.reduce((s, i) => s + i.amount_due, 0)
  const live = db.filter(isLive)
  const thisMonth = TODAY_ISO.slice(0, 7)
  const dueThisMonth = live.filter(
  ...
  const lastExpected = dueThisMonth.map((i) => i.expected_payment_date!).sort().pop()
  const overdue = live.filter(isOverdue).reduce((s, i) => s + i.amount_due, 0)

  const paid = db.filter((i) => i.pipeline_status === 'paid')
  ...
  const pipeline_totals = STAGES.map((st) => {
    const rows = db.filter((i) => i.pipeline_status === st.key)
    return { status: st.key, amount: rows.reduce((s, i) => s + i.total, 0), count: rows.length }
  })
  ...
  return delay({
    ...
    rejected_count: db.filter((i) => i.pipeline_status === 'rejected').length,
    stale_count: live.filter((i) => daysInStage(i) >= 14).length,
    pipeline_totals,
  })
}
...
export function mockCashflow(): Promise<Cashflow> {
  ...
  const out = months.map((m) => {
    const received = db
      ...
      .reduce((s, i) => s + (i.total - i.amount_due) - i.middleman_fee, 0)
    const expected = db
      .filter((i) => isLive(i) && i.expected_payment_date && ym(i.expected_payment_date) === m)
...
export function mockCompare(): Promise<Compare> {
  const rollup = (p: PayerType): PayerRollup => {
    const rows = db.filter((i) => i.payer_type === p)
    ...
    return {
      ...
      total_received: paid.reduce((s, i) => s + (i.total - i.amount_due), 0),
...
/** Expected SLA for an insurer; exported so the demo and Settings agree. */
export const slaFor = (insurer: string): number => SLA_DAYS[insurer] ?? DEFAULT_SLA
